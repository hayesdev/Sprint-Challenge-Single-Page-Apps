...
Client-side routing is a method of using JS to control how content is linked together on a site without having to wait for a server http response every time a link is clicked. The benefits are that it is faster for the user and does not break up/interrupt the user experience.
